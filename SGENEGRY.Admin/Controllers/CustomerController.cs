    ...
    public class CustomerController : Controller
    {
        ...
        public async Task<IActionResult> SaveData(Customer data)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(data.Phone) && !PhoneRegex.IsMatch(data.Phone))
                    ModelState.AddModelError(nameof(data.Phone), "Số điện thoại không đúng định dạng (VD: 0xxxxxxxxx hoặc +84xxxxxxxxx)");
