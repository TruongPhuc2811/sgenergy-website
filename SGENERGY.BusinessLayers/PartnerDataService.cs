...
public static class PartnerDataService
{
    ...
    {
        if (!ValidateSupplierData(data,isNew: true))
            return 0;
    ...
    {
        if (!ValidateSupplierData(data, isNew: false))
            return false;
    ...
    /// </summary>
    /// <param name="data">dữ liệu của nhà cung cấp</param>
    /// <param name="isNew">True nếu là khách hàng mới, ngược lại là False </param>
    /// <returns></returns>
    private static bool ValidateSupplierData(Supplier data, bool isNew)
    {
        if (data == null)
            return false;
        ...
        if (!isNew && data.SupplierID <= 0)
            return false;

        // Email không bắt buộc, nhưng nếu có thì trim
    ...
    {
        if (!ValidateCustomerData(data, isNew: true))
            return 0;
        // Kiểm tra email trùng
        if (!await customerDB.ValidateEmailAsync(data.Email, 0))
            return 0;
    ...
    {
        if (!ValidateCustomerData(data, isNew: false))
            return false;
    ...
    /// <param name="isNew">True là khách hàng mới, false là khách hàng cần cập nhật</param>
    /// <returns></returns>
    private static bool ValidateCustomerData(Customer data, bool isNew)
    {
        if (data == null)
            return false;
        ...
        // Email bắt buộc cho tài khoản khách hàng
        if (string.IsNullOrWhiteSpace(data.Email))
            return false;

        if (!isNew && data.CustomerID <= 0)
            return false;
    ...
    {
        if (!ValidateShipperData(data, isNew: true))
            return 0;
    ...
    {
        if (!ValidateShipperData(data, isNew: false))
            return false;
    ...
    /// <summary>
    /// Kiểm tra dữ liệu người giao hàng
    /// </summary>
    private static bool ValidateShipperData(Shipper data, bool isNew)
    {
        if (data == null)
            return false;
